...
func NewCalculator(hoursPerMonth float64) *Calculator {
	return &Calculator{hoursPerMonth: hoursPerMonth}
}
...
func (c *Calculator) EnrichRecommendations(workload domain.WorkloadMetrics, recommendations []domain.Recommendation, prices pricing.ResourcePricing) []domain.Recommendation {
	for index := range recommendations {
		if recommendations[index].Workload != workloadKey(workload) {
			continue
		}
...
func enrichRecommendation(recommendation *domain.Recommendation, workload domain.WorkloadMetrics, prices pricing.ResourcePricing, hoursPerMonth float64) {
	...
	optimizedCPU := workload.CPURequestMillicores
	optimizedMemory := workload.MemoryRequestBytes
	if recommendation.SuggestedCPURequestMillicores > 0 && recommendation.SuggestedCPURequestMillicores < optimizedCPU {
		optimizedCPU = recommendation.SuggestedCPURequestMillicores
	}
	if recommendation.SuggestedMemoryRequestBytes > 0 && recommendation.SuggestedMemoryRequestBytes < optimizedMemory {
		optimizedMemory = recommendation.SuggestedMemoryRequestBytes
	}
...
func recommendationPriority(annualSavings float64, confidence domain.Confidence) domain.RecommendationPriority {
	switch {
	case annualSavings >= 1000 && confidence == domain.ConfidenceHigh:
		return domain.RecommendationPriorityCritical
	case annualSavings >= 500:
		return domain.RecommendationPriorityHigh
	case annualSavings >= 100:
